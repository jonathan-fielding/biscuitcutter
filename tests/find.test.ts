...
import { findTemplate } from '../src/find';
import { NonTemplatedInputDirError } from '../src/exceptions';

const TESTS_DIR = path.join(__dirname);

describe('findTemplate', () => {
  it('should find template directory with default jinja strings', () => {
    const repoDir = path.join(TESTS_DIR, 'fake-repo-pre');
  ...
  it('should throw NonTemplatedInputDirError when no template dir exists', () => {
    const repoDir = path.join(TESTS_DIR, 'fake-repo-bad');
  ...
  it('should find template in fake-repo-pre2 with alternate delimiters', () => {
    ...
    const repoDir = path.join(TESTS_DIR, 'fake-repo-pre2');
  ...
  it('should find template in ordinary repo', () => {
    const repoDir = path.join(TESTS_DIR, 'test-generate-files');
    const template = findTemplate(repoDir);
